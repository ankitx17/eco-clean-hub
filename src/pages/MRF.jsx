...
function getFacilityTypeLabel(type) {
  ...
  if (normalized.includes("dry waste")) {
    return "Dry Waste Centre"
  }

  if (normalized.includes("recycling")) {
...
function getFacilityTypeBadgeClass(type) {
  const label = getFacilityTypeLabel(type)

  if (label === "E-Waste Facility") {
    return "bg-purple-50 text-purple-700 border-purple-100"
  }

  if (label === "Recycling Centre") {
    return "bg-blue-50 text-blue-700 border-blue-100"
  }

  if (label === "Dry Waste Centre") {
    return "bg-amber-50 text-amber-700 border-amber-100"
  }

  if (label === "Municipal Facility") {
    return "bg-slate-100 text-slate-700 border-slate-200"
  }

  if (label === "Collection Point") {
...
function MRF() {
  ...
  useEffect(() => {
    ...
    const fetchCurrentAddress = async () => {
      ...
      try {
        ...
        setCurrentAddress(
          data.display_name || "Current address unavailable"
        )
      } catch (error) {
        console.error(
          "Current address lookup error:",
          error
        )

        setCurrentAddress(
          "Current address could not be determined"
        )
      } finally {
  ...
  const handleViewRoute = (facility) => {
    sessionStorage.setItem(
      "selectedMRF",
      JSON.stringify(facility)
    )

    navigate(`/mrf/route/${facility.id}`)
  }
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {locationLoading
                    ...
                    : hasLocation
                      ? "Searching verified and mapped waste recovery facilities nearby."
                      ...
                      (facility, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Address */}
                            <div className="mt-5 flex items-start gap-2 rounded-2xl bg-[#f8fbf9] p-3.5">
                              <MapPin
                                size={16}
                                className="mt-0.5 shrink-0 text-[#176b45]"
                              />
                            ...
                            {/* Phone */}
                            {facility.phone && (
                              <div className="mt-3 flex items-center gap-2 rounded-2xl bg-[#f8fbf9] p-3.5">
                                ...
                                <a
                                  ...
                                  className="text-sm font-bold text-[#176b45] hover:underline"
                                >
                                  {facility.phone}
                                </a>
                              </div>
                ...
                nearbyFacilities.length === 0 && (
                  <div className="mt-6 rounded-3xl border border-dashed border-[#cfe1d6] bg-white p-10 text-center">
                    ...
                    <h3 className="mt-4 text-lg font-bold text-slate-800">
                      No waste recovery facilities found nearby
                    </h3>

                    <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
                      We could not find any verified or mapped
                      waste recovery facilities within the current
                      search area.
                    </p>
